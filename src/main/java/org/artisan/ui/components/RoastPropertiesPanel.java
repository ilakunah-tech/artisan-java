...
/**
 * Roast Properties content for RightRail Details panel.
 * Extracted from RoastPropertiesDialog; embeds in dock, no modal.
 */
public final class RoastPropertiesPanel extends VBox {
    ...
    public RoastPropertiesPanel(AppController appController, Runnable onApplied) {
    ...
    private Node buildContent() {
        ...
        customLabelsList.setPrefHeight(80);

        Button addLabelBtn = new Button("Add");
        Button editLabelBtn = new Button("Edit");
        Button removeLabelBtn = new Button("Remove");
        ...
        beanGrid.add(new Label("Custom labels:"), 0, br++);
        ...
        applyBtn.getStyleClass().add("ri5-primary-button");
        applyBtn.setOnAction(e -> apply());

        VBox root = new VBox(8, tabs, applyBtn);
    ...
    private void apply() {
        ...
        if (!RoastPropertiesValidator.showErrors(owner, p)) return;
        copyToController(p);
        p.save();
